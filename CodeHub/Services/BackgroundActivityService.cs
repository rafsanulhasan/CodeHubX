		...
		public async void Run(BackgroundActivatedEventArgs args)
		{
			...
			switch (taskName)
			{
				case "AppTrigger":
					...
					var appTriggerArgs = BackgroundTaskArgument.Parse(appTriggerDetails.Arguments);
				...
				case "ToastNotificationAction":
						...
						await ExecutionService.RunActionInCoreWindow<BackgroundTaskDeferral>(async () =>
					{
						try
						{
							var toastArgs = BackgroundTaskArgument.Parse(QueryString.Parse(toastTriggerDetails.Argument));
