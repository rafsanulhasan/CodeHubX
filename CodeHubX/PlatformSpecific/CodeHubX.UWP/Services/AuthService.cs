...
namespace CodeHubX.UWP.Services
{
	internal class AuthService
	{
		...
		private GitHubClient client = new GitHubClient(new ProductHeaderValue("CodeHubX"));
		...
		public async Task<bool> Authenticate()
		{
			try
			{
				...
				var clientSecret = await AppCredentials.GetAppSecret();
		...
		private async Task<bool> SaveToken(string token, string clientId)
		{
			try
			{
				...
				await AccountsService.AddUser(new CodeHubX.Models.Account { Id = user.Id, AvatarUrl = user.AvatarUrl, IsLoggedIn = true, Login = user.Login, IsActive = true });
