...
using SignOutMessageType = CodeHubX.Helpers.GlobalHelper.SignOutMessageType;
...
namespace CodeHubX.UWP.ViewModels
{
	public class MainViewmodel : AppViewmodel
	{
		...
		private ObservableCollection<CodeHubX.Models.Account> _InactiveAccounts = new ObservableCollection<CodeHubX.Models.Account>();
		public ObservableCollection<CodeHubX.Models.Account> InactiveAccounts
		{
		...
		private CodeHubX.Models.Account _activeAccount = new CodeHubX.Models.Account();
		public CodeHubX.Models.Account ActiveAccount
		{
		...
		public async Task Initialize()
		{
			...
			if (IsLoggedin)
			{
				...
				else if (ActivatedEventArgs is ToastNotificationActivatedEventArgs toastEventArgs)
				{
					...
					if (long.TryParse(args["repoId"], out var repoId))
					{
						...
						switch (args["action"])
						{
							case "showIssue":
								...
								if (int.TryParse(args["issueNumber"], out var issueNumber))
								{
							...
							case "showPr":
								...
								if (int.TryParse(args["prNumber"], out var prNumber))
								{
				...
				BadgeHelper.UpdateBadge(UnreadNotifications?.Count ?? 0);
			}
		}
		...
		{
			await AccountsService.MakeAccountActive((e.ClickedItem as CodeHubX.Models.Account).Id.ToString());
			await InitializeAccounts();
			IsAccountsPanelVisible = false;
		}
		...
		public void MainFrame_Navigated(object sender, NavigationEventArgs e)
			=> HamItems.ForEach(hi =>
			{
				hi.IsSelected = e.SourcePageType == hi.DestPage;
			});
		...
		{
			HamItems.ForEach(hi => hi.IsSelected = false);
