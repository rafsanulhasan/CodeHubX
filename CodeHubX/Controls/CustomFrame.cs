...
namespace CodeHub.Controls
{
	...
	public class CustomFrame : Frame
	{
		...
		public async Task<bool> NavigateWithoutAnimations(Type pageType, [Optional] object parameter)
		{
			var eventPending = IsEmpty;
			var result = await Navigate(pageType, parameter);
			if (eventPending)
			{
		...
		/// <param name="pageType">The type of the target page</param>
		/// <param name="parameter">The optional parameter</param>
		public async Task<bool> Navigate(Type pageType, [Optional] object parameter)
		{
			...
			async void finishNavigation(bool empty)
			{
				...
				Opacity = 0;
				var navResult = await Navigate(pageType, parameter);
				if (empty)
				{
		...
		public void ClearContent(bool skipAnimations = false)
		{
		...
		//public object LastNavigationParameter 
		//	=> BackStackDepth == 0 ? null : BackStack.Last().Parameter;

		//public void IsContentVisible(bool value)
		//	=> FrameworkContent.SetVisualOpacity(value ? 1 : 0);
