...
namespace CodeHubX.Services
{
	public static class NotificationsService
	{
		...
		public static async Task SetThreadSubscription(string notificationId, bool subscribed, bool ignored)
		{
			if (int.TryParse(notificationId, out var id))
			{
				await GlobalHelper.GithubClient.Activity.Notifications.SetThreadSubscription(
					id,
		...
		public static async Task<ThreadSubscription> GetSubscribtionThread(string notificationId)
			...
			? await GlobalHelper.GithubClient.Activity.Notifications.GetThreadSubscription(id)
			: null;
	}
}
